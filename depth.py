...
time.sleep(4)
PATH = os.getcwd()
output_data_dir = 'right'
folder_for_saving = '20'
...
try:
    while True:
        ...
        # cv2.imshow('RealSense', color_image)  #uncomment and comment depth when capturing Depth images
        ...
        cv2.imwrite(output_file_path, depth_colormap)  # saving depth  to defined location
        # cv2.imwrite(output_file_path, color_image) #saving color images to defined location, but file path needs to be changed
        frame_num += 1
finally:
    # Stop streaming
    pipeline.stop()
